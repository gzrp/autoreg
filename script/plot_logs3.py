...
def extract_metric_from_line(line, metric_type):
    ...
    patterns = {
        ...
        "Train BAcc": r"Train Loss:\s*[\d\.]+,\s*Train Acc:\s*[\d\.]+,\s*Train BAcc:\s*([\d\.]+)",

        "Val Loss": r"Val Loss:\s*([\d\.]+)",
        "Val BAcc": r"Val Loss:\s*[\d\.]+,\s*Val Acc:\s*[\d\.]+,\s*Val BAcc:\s*([\d\.]+)",
    }
...
def plot_metrics(directory):
    metrics = ["Train Loss", "Train BAcc", "Val Loss", "Val BAcc"]
...
if __name__ == "__main__":
    log_dir = "/data/ruipeng/workdir/autoreg/script/clickpred"
