...
# Frappe 元数据信息
class FrappeMetaData:
    def __init__(self):
        super().__init__()
        self.dataset_name = "frappe"
        self.instance =  288609
        self.feats = 11
        self.in_features = 5382
        self.out_features = 2
        self.batch_size = 128
        self.is_balanced = False
        self.class_ratio = [134423, 67604]
        self.data_dir = "/.data/frappe"
        self.column_names = [
...
class FrappeProcessor:
    def __init__(self):
        super().__init__()
        self.meta = FrappeMetaData()
    ...
    def process(self, df: pd.DataFrame) -> tuple:
        ...
        # 构造特征类型映射字典
        feature_types = {}
        if self.meta.continuous_features:
            for col in self.meta.continuous_features:
                feature_types[col] = "numerical"
        if self.meta.binary_features:
            for col in self.meta.binary_features:
                feature_types[col] = "binary"
        if self.meta.multiclass_features:
...
class FrappeDataset(Dataset):
    def __init__(self, data_dir, mode: str = "train"):
        ...
        self.mode = mode
        self.processor = FrappeProcessor()
...
def get_frappe_dataset(data_dir):
    train_set = FrappeDataset(mode="train", data_dir=data_dir)
    val_set = FrappeDataset(mode="val", data_dir=data_dir)
    test_set = FrappeDataset(mode="test", data_dir=data_dir)
    return train_set, val_set, test_set

def get_frappe_dataset_sampled(data_dir, sample_ratio=0.2):
    train_set = FrappeDataset(mode="train", data_dir=data_dir)
    val_set = FrappeDataset(mode="val", data_dir=data_dir)
    test_set = FrappeDataset(mode="test", data_dir=data_dir)
...
def get_frappe_dataloader(data_dir, batch_size):
    train_set = FrappeDataset(mode="train", data_dir=data_dir)
    val_set = FrappeDataset(mode="val", data_dir=data_dir)
    test_set = FrappeDataset(mode="test", data_dir=data_dir)
...
def get_frappe_dataloader_sampled(data_dir, batch_size, sample_ratio=0.2):
    train_set = FrappeDataset(mode="train", data_dir=data_dir)
    val_set = FrappeDataset(mode="val", data_dir=data_dir)
    test_set = FrappeDataset(mode="test", data_dir=data_dir)
    # 分层按比例采样（推荐）
...
if __name__ == '__main__':
    ...
    # 原始数据
    train_set_full = FrappeDataset(mode="train", data_dir=data_dir)

    # 采样后数据
    train_loader, val_loader, test_loader = get_frappe_dataloader_sampled(
        data_dir=data_dir,
