...
def exploitation_train(config, args, train_set, val_set, test_set, first_load_time):
    ...
    for epoch in range(max_epochs):
        ...
        metrics = {
            ...
            "first_load_time_real": first_load_time,
            "load_data_time": load_data_time_total,
            "load_data_time_real": load_data_time_total,
            "with_cache_data_time": load_data_time_total,
            "without_cache_data_time": first_load_time + load_data_time_total,
            "loss": loss,
            "acc": acc_max,
            "auc": auc_max,
...
def parse_results(df: pd.DataFrame):
    ...
    for _, row in df_sorted.iterrows():
        ...
        items.append({
            ...
            "first_load_time_real": row["first_load_time"],
            "load_data_time": row["load_data_time"],
            "load_data_time_real": row["load_data_time_real"],
            "with_cache_data_time": row["with_cache_data_time"],
            "without_cache_data_time": row["without_cache_data_time"],
            "loss": row["loss"],
            "acc": row["acc"],
            "auc": row["auc"],
...
class ExploitPhase:
    def __init__(self, args):
        ...
        time2 = time.time()
        self.first_load_time = time2 - time1
        ...
        tuner = Tuner(
            trainable=tune.with_resources(
                tune.with_parameters(exploitation_train, args=self.args, train_set=self.train_set, val_set=self.val_set,
                                     test_set=self.test_set, first_load_time=self.first_load_time,),
...
if __name__ == '__main__':
    ...
    # 用法
    write_list_to_jsonl(all_res,
                        f"/data/ruipeng/workdir/autoreg/.exp_results/exp6_2/{args.dataset}/explore_result_{args.num_samples}.jsonl",
                        append=False)
    save_dict_to_file(data=save_result, base_dir=f"/data/ruipeng/workdir/autoreg/.exp_results/exp6_2/{args.dataset}",
                      prefix=f"{args.exp_name}_explore_{args.num_samples}")
    ...
    if C>0:
        ...
        save_result2 = {
            ...
            "total_time": exploit_time + exploit_time,
            "exploit_num": len(res2),
            "exploit_result": res2,
        }
        res2 = numpy_to_python(res2)
        # 用法
        write_list_to_jsonl(res2,
                            f"/data/ruipeng/workdir/autoreg/.exp_results/exp6_2/{args.dataset}/exploit_result_{args.num_samples}.jsonl",
                            append=False)
        save_dict_to_file(data=save_result2,
                          base_dir=f"/data/ruipeng/workdir/autoreg/.exp_results/exp6_2/{args.dataset}",
                          prefix=f"{args.exp_name}_exploit_{args.num_samples}")
