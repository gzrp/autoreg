...
6600 7200 8400 9600 10800 11912
)

# 顺序执行
for b in "${budgets[@]}"; do
    echo "🚀 正在运行: python exp_random_2phase.py --budget=${b}"
    python ../exp_random_2phase.py --dataset=connect --batch_size=64 --max_epochs=16 --grace_period=1 --num_workers=4 --budget=${b} --device_ids=0,1 --gpu_ids=0,1,0,1
